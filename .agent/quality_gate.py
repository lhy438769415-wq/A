...
# DDL 单一来源：只允许 core/database.py 出现 CREATE TABLE
DDL_FILE = os.path.join('core', 'database.py')
...
def scan_ddl_violations(files):
    ...
    for fp in files:
        rel = os.path.relpath(fp, PROJECT_ROOT)
        if rel.replace('\\', '/') == DDL_FILE.replace('\\', '/'):
            continue
        try:
            with open(fp, encoding='utf-8') as fh:
                for i, line in enumerate(fh, 1):
                    if re.search(r'CREATE\s+TABLE', line, re.I):
                        hits.append((rel, i, 'CREATE TABLE outside database.py',
                                     line.strip()))
        except (OSError, UnicodeDecodeError):
            pass
    return hits
...
def main():
    ...
    if ddl:
        print(f'[违规] DDL 非单一来源 {len(ddl)} 处:')
        for rel, i, name, snippet in ddl:
            print(f'  {rel}:{i}  {snippet[:80]}')
    else:
        print('[OK] CREATE TABLE 仅出现在 core/database.py')
    ...
    if forbidden or (strict and ddl):
        print('结果: 未通过 (退出码 1)')
        sys.exit(1)
    print('结果: 通过')
    sys.exit(0)
