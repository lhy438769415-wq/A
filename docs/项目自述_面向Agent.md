...
| GAP-H2 / MTR 策略卡 + 示意图 | ✅ **已完成**（09-22）：两份“六类交易员视角”策略卡（`docs/gap_h2_strategy_card.html` / `docs/mtr_strategy_card.html`，逐条对照生产代码核实、纠正旧 spec 漂移）+ 两张利旧 `notifier` 出图工具的典型形态示意图 PNG，已嵌入卡内并入库 |
