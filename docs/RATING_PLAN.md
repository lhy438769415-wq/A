...
- [ ] **Phase 2 数据闭环（P0-2 已落地 + P0-3 口径重定）**：
  - 生产评级现已加载 `config/rating_factors.json`（带缺失/损坏→回退手调阈值兜底）；`calibrated` 标志由"文件是否可用"决定，不再硬编码（awil 已修正）。
  - **验收口径重定（2026-07-26，第三方评审 P0-3 →  traders equation 校正）**：原"A+ 毛胜率 ≥55%"在跨年 Walk-Forward 净口径下未达成（A+ 跨年净胜率 48.69%、EV -0.016R），故废弃"胜率硬门槛"。**关键校正（用户指出）**：价格行为学的数学理论基础是 **Al Brooks 交易者方程 / Trader's Equation**：`EV = 胜率 × 平均盈利R − (1−胜率) × 平均亏损R`。**胜率不是评级标准，数学期望(EV)>0 才是。** Brooks 明训：40% 胜率 + 2R 盈亏比 = 长期正期望（10 笔赚 4×2R、亏 6×1R → 净 +2R）；做波段可接受 40–50% 胜率，但盈亏比须 ≥2:1，R:R<1:1 直接 pass。
  - **新验收判据 = 逐策略、EV 驱动**：以「跨年 Walk-Forward **净 EV > 0（以 R 计）**」为唯一及格线，胜率仅作参考（低胜率 + 高盈亏比亦可及格）。字母切点由校准在训练集反推（A+ = 该策略 EV 最高且 >0 的得分段）。
  - 校准已用修复后的真周线数据重跑（`core/data_provider.get_stock_data` 的 `timeframe='weekly'` 分支此前被忽略，P0-1 已修），三周线策略校准结论已重建。
  - **全历史回测 EV 汇总（2026-07-26，按交易者方程；源 = `calibration_signals.jsonl` 全量已结案信号，EV=均值净R）。数据范围：日线 `daily_bars` 仅 2023-01-03→2026-07-24（≈3.5 年）；周线 `weekly_bars` 2011-03-04→2026-07-24（≈15 年）。标池=3310 当前上市股（幸存者偏差，已退市股未纳入）。**
    - **日线级别（全历史 2023+，样本偏短，小 n 策略置信低）**：
      | 策略 | n | 胜率 | **整体 EV** | A+ n | A+ 胜率 | **A+ EV** |
      |---|---|---|---|---|---|---|
      | 缺口H2 | 2209 | 42.0% | **+0.080R** | 77 | 39.0% | −0.014R |
      | 3K | 398 | 53.5% | **+0.057R** | 374 | 52.4% | +0.049R |
      | MTR | 85704 | 48.4% | **+0.042R** | 173 | 55.5% | **+0.133R** |
      | 结构性缺口 | 4738 | 44.6% | **+0.005R** | 113 | 54.0% | −0.069R |
      | 缺口Pinbar | 269 | 33.8% | −0.033R | 96 | 39.6% | +0.155R |
      | AWIL | 520 | 36.3% | −0.052R | 348 | 37.9% | −0.009R |
    - **周线级别（全历史 2011+，≈15 年，稳健）**：
      | 策略 | n | 胜率 | **整体 EV** | A+ n | A+ 胜率 | **A+ EV** |
      |---|---|---|---|---|---|---|
      | 缺口Pinbar | 126 | 49.2% | **+0.482R** | 74 | 55.4% | **+0.475R** |
      | 缺口H2 | 1240 | 55.1% | **+0.381R** | 158 | 64.6% | **+0.380R** |
      | 结构性缺口 | 3744 | 58.5% | **+0.295R** | 658 | 71.0% | **+0.253R** |
    - **结论（全历史口径，EV>0 为及格）**：
      - **周线缺口家族三策略全部强正 EV（+0.30~+0.48R），15 年稳健**——这是系统真正的 edge，应作为主力推送周期。
      - **日线普遍弱**：仅缺口H2/3K/MTR 整体微正（+0.04~0.08R），结构性缺口日线≈持平（+0.005R），Pinbar日线/AWIL日线为负。**结构性缺口应优先周线交易（周 +0.253R A+ vs 日 −0.069R A+）——同策略跨周期结论可相反。**
      - **A+ 档与整体可背离**：缺口Pinbar日线整体负但 A+ +0.155R（优）；结构性缺口日线 A+ −0.069R（劣）；说明字母分级在日线噪声下不稳定，**周线字母分级更可信**。
      - **校正此前样本外误判**：早前 `--annual-cv` 测试折给出 `mtr_daily` A+ −0.016R（判负期望），但全历史 A+ = **+0.133R（正）**；该负值为单折/特定regime假象，非全历史结论。验收以全历史 EV 为准。
    - **方法学保留偏差（验收时需带）**：① **幸存者偏差**：标池仅 3310 当前上市股，15 年周线 EV 偏乐观（已退市弱势股未计入）；② **日线历史仅 3.5 年**，小 n 策略（Pinbar日 n=269、AWIL n=520、3K n=398）置信区间宽；③ **生命周期三过滤仅缺口家族启用**（INVALIDATED/VOIDED/TIMEOUT 共排除 8268 笔），MTR/3K/AWIL 不启用→分母口径不一致，跨策略 EV 不可机械直接比；④ **滑点=0**（Buy-Stop 跳空入场实际有不利滑点），报告 EV 为上限；⑤ 部分交易因未触止盈/止损成 HOLDING（1120 笔）被剔出分母，轻微偏差。
  - **后续（校准产物增强）**：当前 `rating_factors.json` 仅存各字母胜率证据，缺 EV / 平均盈利R / 平均亏损R。应让校准脚本额外吐 `ev_per_letter` 与 `avg_win_r`/`avg_loss_r`，使生产推送可直接展示"该字母历史 EV"，真正落地交易者方程（而非只秀胜率）。
