...
| 版本 | 日期 | 主要变更 |
|:---:|:---:|:---|
| V10.0 | 2026-07-25 | **架构收敛 + 工程守门 + 高可用（质变版本）**：①彻底单引擎——周线/3K 扫描入口统一到 `hunter.py` 单一 CLI + `core/scan_engine` 共享编排，删除两个重复 scanner 脚本，等价 diff 零逻辑漂移；②自动门禁 hook（pre-commit 自动跑 quality_gate，红线/DDL/测试数守卫拦截，测试基线 143→156）；③运行监控/崩溃告警（hunter 入口崩溃兜底发 Discord + crash_log，心跳 last_run.json，新增 check_heartbeat.py）；④配套：死代码归档、项目代码全书、文档收尾、全流程验证 156 测试全绿、0 红线。 |
